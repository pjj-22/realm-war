...
export async function notifyIncomingAttack(attackerId, toHex, quantity, arrivesAt) {
  try {
    ...
    // This is always a warning about the recipient's own hex, so it's always
    // fully visible to them (server/visibility.js canSeeDetail's isOwner
    // bypass) regardless of darkness - no need to mask or hide the quantity.
