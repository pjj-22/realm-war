...
- **Logs**: `docker compose logs backend -f` (or `--tail N`). Default level is `info` in prod - registration, login (incl. failed attempts, at `warn`), account deletion, rate-limit trips, and every admin action are always in there. Set `LOG_LEVEL=debug` in `.env` and `docker compose up -d backend` to add a line per HTTP request plus tick/bot chatter for a specific debugging session - it's noisy, don't leave it on. See `server/logger.js` / `CLAUDE.md`.
...
- **Rate limits key on `CLIENT_IP_HEADER`** (`cf-connecting-ip`, the compose default) rather than `req.ip`. Traffic arrives Cloudflare → cloudflared → nginx → backend, and nginx appends cloudflared's address to `X-Forwarded-For`, so with `TRUST_PROXY=1` `req.ip` is cloudflared's address for *every* request - one shared bucket for all players (10 registrations/hour total, not per person). Cloudflare overwrites `CF-Connecting-IP` at the edge, so it's the trustworthy one here. Admin routes only count *failed* secret checks toward their brute-force limit (20 per 15 min); the portal's own polling no longer locks it out.
