...
- **nginx `mime.types` has no `.mjs` entry.** MapLibre 6's module worker ships as a hashed `.mjs` asset; served as `application/octet-stream`, Firefox refuses to start it and the map is black (Chromium is lenient, so it looks fine there). `client/nginx.conf` has a dedicated `location ~* \.mjs$` for it, and the Deploy workflow smoke-tests every built JS asset's content-type out of the real image before pushing - CI's e2e runs on `vite preview`, whose own MIME table won't catch this.
