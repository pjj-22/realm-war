const BASE = (import.meta.env.VITE_API_URL || 'http://localhost:3001') + '/api'

async function request(method, path, body, { allowNull } = {}) {
  const token = localStorage.getItem('rw_token')
  const res = await fetch(`${BASE}${path}`, {
    method,
    headers: {
      'Content-Type': 'application/json',
      ...(token ? { Authorization: `Bearer ${token}` } : {}),
    },
    ...(body ? { body: JSON.stringify(body) } : {}),
  })
  const data = await res.json()
  if (!res.ok) {
    if (allowNull) return null
    throw new Error(data.error || 'Request failed')
  }
  return data
}

export const api = {
  register: (username, password, color, ageConfirmed) => request('POST', '/players/register', { username, password, color, ageConfirmed }),
  deleteAccount: () => request('DELETE', '/players/me'),
  exportData: () => request('GET', '/players/export'),
  login: (username, password) => request('POST', '/players/login', { username, password }),
  me: () => request('GET', '/players/me'),
  saveFlag: (flagPixels, motto) => request('POST', '/players/flag', { flagPixels, motto }),
  getLeaderboard: () => request('GET', '/players/leaderboard'),
  searchPlayers: (q) => request('GET', `/players/search?q=${encodeURIComponent(q)}`),
  getEmpire: () => request('GET', '/players/empire'),
  getStats: () => request('GET', '/players/stats'),
  claimHex: (h3Index) => request('POST', '/hexes/claim', { h3Index }),
  setCapital: (h3Index) => request('POST', '/hexes/set-capital', { h3Index }),
  getHexesViewport: (h3Indexes) => request('POST', '/hexes/viewport', { h3Indexes }),
  getMyHexes: () => request('GET', '/hexes/mine'),
  getHexOverview: (res) => request('GET', `/hexes/overview?res=${res}`),
  searchHexCode: (code) => request('GET', `/hexes/search?code=${encodeURIComponent(code)}`, null, { allowNull: true }),
  getBuildings: (h3Index) => request('GET', `/buildings/${h3Index}`),
  build: (h3Index, type) => request('POST', '/buildings', { h3Index, type }),
  demolish: (buildingId) => request('DELETE', `/buildings/${buildingId}`),
  upgradeHex: (h3Index) => request('POST', `/buildings/${h3Index}/upgrade`),
  getMilitary: (h3Index) => request('GET', `/military/hex/${h3Index}`),
  trainTroops: (h3Index, type, quantity) => request('POST', '/military/train', { h3Index, type, quantity }),
  marchArmy: (fromHex, toHex, type, quantity) => request('POST', '/military/march', { fromHex, toHex, type, quantity }),
  setOrders: (h3Indexes, min_troops, build = null) => request('POST', '/military/orders', { h3Indexes, min_troops, build }),
  marchMany: (sources, toHex, keep, sync = false) => request('POST', '/military/march-many', { sources, toHex, keep, sync }),
  reinforce: (keep, dryRun = false) => request('POST', '/military/reinforce', { keep, dryRun }),
  fanOut: (sources, keep, mode, dryRun = false, sync = false) => request('POST', '/military/fan-out', { sources, keep, mode, dryRun, sync }),
  getArmies: () => request('GET', '/military/armies'),
  getPendingClaims: () => request('GET', '/military/pending-claims'),
  recallArmy: (id) => request('DELETE', `/military/armies/${id}`),
  checkTerrain: (h3Indexes) => request('POST', '/hexes/terrain', { h3Indexes }),
  getRoute: (fromHex, toHex) => request('POST', '/hexes/route', { fromHex, toHex }),
  getBattle: (h3Index) => request('GET', `/battles/hex/${h3Index}`),
  getActiveBattles: () => request('GET', '/battles/active'),
  getEvents: () => request('GET', '/events'),
  peekEvents: () => request('GET', '/events?peek=1'),
  getEventCount: () => request('GET', '/events/count'),
  devRefill: () => request('POST', '/dev/refill'),
  getConfig: () => request('GET', '/health'),
  getStrategicHexes: () => request('GET', '/hexes/strategic'),
  getHistory: () => request('GET', '/players/history'),
  setRally: (fromHex, rallyHex) => request('POST', '/military/rally', { fromHex, rallyHex }),
  clearRally: (h3Index) => request('DELETE', `/military/rally/${h3Index}`),
  suggestStart: () => request('GET', '/hexes/suggest-start'),
  getWorldEvents: () => request('GET', '/world/events'),
  getCrowns: () => request('GET', '/world/crowns'),
  getZones: () => request('GET', '/world/zones'),
  getWonders: () => request('GET', '/world/wonders'),
  getMonuments: () => request('GET', '/world/monuments'),
  getPushKey: () => request('GET', '/push/key'),
  pushSubscribe: (subscription) => request('POST', '/push/subscribe', { subscription }),
  pushUnsubscribe: (endpoint) => request('DELETE', '/push/subscribe', { endpoint }),
  getMyAlliance: () => request('GET', '/alliance/mine'),
  createAlliance: (name, tag) => request('POST', '/alliance/create', { name, tag }),
  joinAlliance: (code) => request('POST', '/alliance/join', { code }),
  leaveAlliance: () => request('POST', '/alliance/leave'),
  getChat: (channel) => request('GET', `/chat?channel=${channel}`),
  sendChat: (channel, text) => request('POST', '/chat', { channel, text }),
  sendFeedback: (category, message) => request('POST', '/feedback', { category, message }),
  getSeason: () => request('GET', '/seasons/current'),
  getSeasonHistory: () => request('GET', '/seasons/history'),
}
