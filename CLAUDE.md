...
- **Logging (`logger.js`)**: `createLogger(tag)` gives `.debug/.info/.warn/.error`, one line per call to stdout/stderr (`docker compose logs backend`/journald capture it as before - no shipper). `LOG_LEVEL` (unset follows `MODE`: `info` in prod, `debug` in dev/test) is the floor; `error < warn < info < debug`. `debug` is per-request access logging (`requestLogger.js`, excludes `/api/health`) plus tick/bot per-action chatter - set `LOG_LEVEL=debug` on the droplet to see either without touching `MODE`/pacing. `info`+ always includes registration, login (success and failed-attempt, at `warn`), account deletion, rate-limit trips (`ratelimit.js`, logged once per bucket that tips over, not every request after), and every mutating admin route (`routes/admin.js` - there's no per-admin identity, just the shared secret, so these log the client IP). New code uses `createLogger`, never a bare `console.*`.
